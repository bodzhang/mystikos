...
static long _syscall(void* args_)
{
    ...
    switch (n)
    {
        ...
        case SYS_exit:
        {
            ...
            if (process->clone.flags & CLONE_VFORK)
            {
                myst_thread_t* parent_launch_thread =
                    myst_find_process_specific_thread(
                        thread->clone.vfork_parent_pid,
                        thread->clone.vfork_parent_tid);
                if (parent_launch_thread)
                {
                    __sync_val_compare_and_swap(
                        &parent_launch_thread->fork_exec_futex_wait, 0, 1);
                    myst_futex_wake(
                        &parent_launch_thread->fork_exec_futex_wait, 1);
                }
            }
