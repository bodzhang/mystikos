...
USAGE="$0 path-to-myst null|exec-sgx|exec-linux config.json timeout ext2|cpio|package <test_list_file> <expected-exitcode>"
...
if [[ "$5" != "package" ]]; then
	...
	if [[ "$3" == "config_8g.json" ]] | [[ "$3" == "config_20g.json" ]]; then
		NPROCS=1
	fi
fi
